...
// Define Exchange type for TypeScript
interface Exchange {
  name: string;
  network: string;
  logo: string;
...
const Exchanges = forwardRef<HTMLDivElement>((props, ref) => (
  ...
  >
    ...
    <div className="container mx-auto px-4 max-w-6xl relative">
      ...
      {/* Exchange cards in a more modern layout */}
...
// New completely redesigned Exchange Card Component 
const ExchangeCard = ({ exchange, index }: { exchange: Exchange; index: number }) => {
  // Create unique action style for each exchange
  const actionStyles = [
  ...
  // Custom shorter labels for each DEX
  ...
  return (
    ...
    >
      {/* Horizontal card layout with logo to the left */}
      <div className="relative h-full group">
        <div className="relative h-full rounded-xl border border-gray-800 hover:border-pink-500/20 transition-all duration-300 overflow-hidden">
          ...
          <div className="relative p-4 md:p-5 flex flex-row items-center">
            {/* Logo container */}
            <div className={`flex-shrink-0 relative bg-gradient-to-br ${
              index % 3 === 0 ? "from-pink-900/30 to-black" : 
